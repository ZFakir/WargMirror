...
const evaluateSift = async (req, res) => {
  try {
    if (!req.files || !req.files.image || !req.files.archival_image) {
      return res.status(400).json({ error: 'Missing required files: image and archival_image' });
    }

    const imageFile = req.files.image[0];
    const archivalImageFile = req.files.archival_image[0];

    const formData = new FormData();
    formData.append('image', new Blob([imageFile.buffer], { type: imageFile.mimetype }), imageFile.originalname);
    formData.append('archival_image', new Blob([archivalImageFile.buffer], { type: archivalImageFile.mimetype }), archivalImageFile.originalname);
    ...
    console.error('Error in evaluateSift:', error);
    return res.status(500).json({ error: 'Failed to process SIFT evaluation' });
  }
};

const evaluateSymmetry = async (req, res) => {
  try {
    if (!req.files || !req.files.image) {
      return res.status(400).json({ error: 'Missing required file: image' });
    }
    ...
    console.error('Error in evaluateSymmetry:', error);
    return res.status(500).json({ error: 'Failed to process symmetry evaluation' });
  }
};
